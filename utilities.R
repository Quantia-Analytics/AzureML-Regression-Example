## This code contains some utility functions used in several Execute R Script
## modules. This file should be zipped and uploaded as a dataset into Azure ML
## Studio. Each Execute R Script model which uses these utilities imports them
## using the R source() function. 


fact.conv <- function(inVec){
  outVec <- as.factor(inVec)
  levels(outVec) <- c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday")
  outVec
}


get.date <- function(Date){
  ## Funciton returns the data as a character string from a POSIXct datatime object. 
  strftime(Date, format = "%Y-%m-%d %H:%M:%S")
}


POSIX.date <- function(Date,Hour){
  ## Function returns POSIXct time series object from date and hour arguments.
  as.POSIXct(strptime(paste(Date, " ", as.character(Hour), ":00:00", sep = ""), "%Y-%m-%d %H:%M:%S"))
}
