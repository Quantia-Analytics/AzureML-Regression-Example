## This code removes downside outliers from the training sample of the bike rental
## data. The value of Quantile variable can be changed to change the trim level.
...
# BikeShare <- BikeShare[BikeShare$monthCount == 13, ]
...
quantByPer <- (
  BikeShare %>%
    ...
    summarise(Quant = quantile(cnt, probs = Quantile, na.rm = TRUE)) 
)
...
## the data frame is allocated so this in-place operation should not be 
## too slow. 
for(month in 1:48){
  for(hour in 0:47){
    indFrame$ind[indFrame$workTime == hour & indFrame$monthCount == month] <- 
      BikeShare$cnt[BikeShare$workTime == hour & BikeShare$monthCount == month] > 
      quantByPer$Quant[quantByPer$workTime == hour & quantByPer$monthCount == month]
  }
}

BikeShare$dteday <- as.POSIXct(strptime(paste(BikeShare$dteday, "00:00:00", sep = ""), "%Y-%m-%d %H:%M:%S"))
BikeShare <- BikeShare[indFrame$ind, ] ## Filter the rows we want
